...
public class MainApp {
  ...
  private static void extracted() {
    ...
    // 添加约束
    manager
        .addConstraint(new SolarExclusionEventHandler(maxIter, maxCheck, threshold, 5.0, 60))
    ...
    List<TimeInterval> initialIntervals = new ArrayList<>();

    // 设置初始时间段
    AbsoluteDate startDate = new AbsoluteDate(2025, 4, 5, 0, 0, 0.0, TimeScalesFactory.getUTC());
    AbsoluteDate endDate   = new AbsoluteDate(2025, 4, 25, 0, 0, 0.0, TimeScalesFactory.getUTC());
    initialIntervals.add(new TimeInterval(startDate, endDate));

    // 执行约束检查（带进度回调）
    List<TimeInterval> validIntervals = manager.executeConstraints(satellitePropagator, targetPropagator, initialIntervals, new ConstraintManager.ProgressCallback() {
      @Override
      ...
      @Override
      public void onConstraintComplete(String name, int current, int total, List<TimeInterval> results) {
        log.warn("约束{}执行完成 ({}/{})，剩余{}个时间窗口", name, current, total, results.size());
      }

      @Override
      public void onComplete(List<TimeInterval> finalResults) {
        log.warn("约束检查完成，最终有{}个有效时间窗口", finalResults.size());
        for (TimeInterval interval : finalResults) {
          log.warn("有效时间窗口：{} 至 {}", 
