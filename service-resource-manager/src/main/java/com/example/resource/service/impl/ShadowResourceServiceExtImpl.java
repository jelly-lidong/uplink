...
public class ShadowResourceServiceExtImpl implements ShadowResourceServiceExt {
        ...
        try {
            ...
            for (TaskSchedule primarySchedule : primarySchedules) {
                ...
                for (String taskType : supportedTypes) {
                    // 使用QueryWrapper根据任务类型查询
                    List<TaskInfo> tasksOfType = taskInfoMapper.selectList(
                            new LambdaQueryWrapper<TaskInfo>()
                                    .eq(TaskInfo::getTaskType, taskType));
    ...
    public boolean isShadowResourceAvailable(GroundEquipment equipment, LocalDateTime startTime, LocalDateTime endTime) {
        try {
            ...
            // 4. 检查是否有重叠的调度计划但仍有并行能力
            return equipment.getMaxParallelTasks() > schedules.size();
    ...
    public TaskSchedule allocateShadowResource(TaskInfo task, GroundEquipment equipment, LocalDateTime startTime, LocalDateTime endTime) {
        try {
            ...
            schedule.setHasShadowRelation(true);
