...
public class SecondsFunction implements Function {
    ...
    public Object execute(Object... args) throws ProtocolException {
        if (args.length != 2) {
            throw new ProtocolException("Seconds function requires 2 arguments");
        }
        
        try {
            LocalDateTime time1 = parseDateTime(args[0]);
            LocalDateTime time2 = parseDateTime(args[1]);
            return ChronoUnit.SECONDS.between(time2, time1);
        } catch (Exception e) {
            throw new ProtocolException("Failed to calculate seconds", e);
        }
    }
